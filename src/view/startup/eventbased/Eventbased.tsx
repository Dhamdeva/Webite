import { Business } from "../../layout/Layout";

export function Eventbased(){
    return(
        <div className="container-fluid">
            <div className="col-lg-11 mx-auto">
                <div className="row">
                    <div className="col-md-7">
                        <Business
                        h1={"Easiest way to mining your Business with PayPe"}
                        p={"We are highly focused on business services and that will lead positively to the future web with PayPe."}
                        />
                    </div>
                    <div className="col-md-5">
                        <img className="img-fluid" src="https://sharkstriker.com/yphathod/2023/03/Understanding-Cybersecurity-compliance-1024x763-mins.webp" alt=".." />
                    </div>
                </div>
            </div>
        </div>
    )
}